...
import { exportYearAheadReportPdf } from "@/lib/artifacts/year-ahead/pdf-export";
...
function errorResponse(error: unknown): NextResponse {
  ...
  return NextResponse.json({ success: false, error: "Year Ahead PDF export failed closed." }, { status: 500 });
}
...
export async function POST(request: Request, { params }: { params: Promise<{ reportId: string }> }) {
  try {
    ...
    if (!parsed.success) return NextResponse.json({ success: false, error: "Invalid paper size." }, { status: 400 });
    const record = getYearAheadWorkflowRecord(reportId);
    const result = await exportYearAheadReportPdf(record.artifact, parsed.data.paperSize);
    const exportedAt = new Date().toISOString();
    recordYearAheadExport(reportId, { documentKind: "report", size: parsed.data.paperSize, fileName: result.fileName, bytes: result.bytes.byteLength, sha256: result.sha256, exportedBy: actorId, exportedAt });
