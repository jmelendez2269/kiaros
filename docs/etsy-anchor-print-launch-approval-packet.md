...
| Retention operation | Base schema ready; redesign migration pending | Production migration `0045` is installed and empty. Additive migration `0046_natal_report_package.sql` exists only in local source and must not be applied without separate approval. A paid scheduler is not required for the first orders. |
