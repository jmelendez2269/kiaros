...
- Inventoried all Etsy/artifact feature flags, admin access layers, and intake/delivery/publication controls in preparation for controlled founder offer → accessibility/legal → publish sequence. Created `docs/ops/etsy-anchor-manual-delivery-retention.md` covering:
  - **Production status verified 2026-09-24:** All three artifact flags (`KIAROS_ETSY_ARTIFACT_ADMIN`, `KIAROS_ETSY_ARTIFACT_PERSISTENCE`, `KIAROS_ETSY_ARTIFACT_REAL_INTAKE`) are enabled in Vercel production. Product `KAI-ETSY-ANCHOR-V1` remains inactive (`active=false` in `artifact_products`). Etsy listing remains unpublished (draft on Etsy Seller Dashboard). Real buyer order processing held by operator discipline (fictional checkbox + clearly fake `DRYRUN-*` IDs only).
  - Named operator enforcement via Clerk `publicMetadata.isAdmin` (primary gate, no separate env allowlist) and `artifact_admin_roles` table (schema exists, not actively enforced in routes yet).
  - Admin UI location: `app/(admin)/admin/artifacts` → `/admin/artifacts` route (durable on main). API route: `/api/admin/artifacts/[orderId]/actions`.
  - Production dry-run click path for manual intake using clearly fake `DRYRUN-*` receipt/transaction IDs + fictional checkbox (fixtures are local-dev only, not available in production).
  - Manual Etsy Messages delivery protocol: what to attach (4 PDFs: Report + Anchor Print, Letter + A4), what to say (template message), post-delivery workflow actions.
  - Retention schedule aligned with published Privacy Policy and Terms: 30 days raw personalization, 180 days normalized/files, 12 months support correspondence, 7 years minimal ledger. Manual `runLocalArtifactRetention()` procedure until volume justifies Vercel cron automation.
  - Explicit statements: listing publication and product activation remain held until all approval gates in `docs/etsy-anchor-print-launch-approval-packet.md` close (founder offer ✅ approved; legal/accessibility review pending).
- Three artifact flags inventoried and production status verified: ADMIN + PERSISTENCE + REAL_INTAKE all `"true"` in Vercel production. Real buyer processing held via inactive product row + unpublished listing + operator discipline (fictional checkbox + fake IDs only).
- Four legacy Etsy SKU/listing mapping vars (`ETSY_SKU_PLANNER`, `ETSY_SKU_PLANNER_ORACLE`, `ETSY_LISTING_ID_*`) and `N8N_INGEST_SECRET` documented as separate activation path, not artifact fulfillment — important distinction to prevent operator confusion.
- Production dry-run workflow: manual intake with `DRYRUN-SHOP-001` / `DRYRUN-RECEIPT-001` / `DRYRUN-TX-001` + `dryrun@example.test` email + fictional checkbox checked → validate → generate → QA (6 items) → export (4 PDFs) → download for review. No Etsy publication, no product activation, no real buyer data.
- Retention implementation: `lib/artifacts/fulfillment/retention.ts` with `runLocalArtifactRetention()`, `artifact_retention_candidates` RPC, and documented manual procedure until order volume justifies automation.
- Updated ETSY-03 / ARTIFACTS tracker status: controlled path in progress as of 2026-09-24, founder offer approved (26-page report, $34, separate Anchor Print), a11y/legal still pending Kai/Jack, publish hold. SOP and flag inventory complete; next gate is independent accessibility review + legal review closing + product activation + listing publication approval.
- This session was docs-only: no app code changes, no migrations, no flag changes (already enabled), no product activation, no Stripe/Etsy API calls, no Clerk user creation.
