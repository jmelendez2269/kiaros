...
- Inventoried all Etsy/artifact feature flags, admin access layers, and intake/delivery/publication controls in preparation for controlled founder offer → accessibility/legal → publish sequence. Created `docs/ops/etsy-anchor-manual-delivery-retention.md` covering:
  - Complete flag inventory distinguishing (a) internal admin/persistence flags safe for named operators vs (b) real buyer intake / Etsy publication / delivery flags that must stay OFF until separate approval.
  - Named operator enforcement via Clerk `publicMetadata.isAdmin` (primary gate) and `artifact_admin_roles` table (schema exists, not actively enforced in routes yet).
  - Admin dry-run click path for one fictional generate → QA → download cycle using `createFixture()` without touching real data or publishing to Etsy.
  - Manual Etsy Messages delivery protocol: what to attach, what to say, post-delivery workflow actions.
  - Retention schedule aligned with published Privacy Policy and Terms: 30 days raw personalization, 180 days normalized/files, 12 months support correspondence, 7 years minimal ledger. Manual `runLocalArtifactRetention()` procedure until volume justifies automation.
  - Explicit statements that listing publication and real buyer intake remain OFF until Kai/Jack close all approval gates in `docs/etsy-anchor-print-launch-approval-packet.md`.
- Three artifact admin flags inventoried: `KIAROS_ETSY_ARTIFACT_ADMIN` (UI visibility), `KIAROS_ETSY_ARTIFACT_PERSISTENCE` (Supabase vs memory), `KIAROS_ETSY_ARTIFACT_REAL_INTAKE` (manual real orders vs fictional fixtures). Combined gate: production requires ADMIN + PERSISTENCE on, REAL_INTAKE stays off for controlled dry-run.
...
- Admin routes documented: `/admin/artifacts` (Anchor Print, gates: admin + persistence), `/admin/artifacts/star-origin` (dev-only), `/admin/artifacts/year-ahead` (dev-only), `/admin/artifacts/celestial-connection` (dev-only). API route: `/api/admin/artifacts/[orderId]/actions` (workflow actions).
- Fictional fixture dry-run flow: `createFixture("known_time" | "unknown_time")` → validate → generate → QA (6 items) → export (4 PDFs) → download for review. No Etsy publication, no real buyer data, no automated delivery in dry-run.
- Real delivery flow (after approval): manual Etsy Messages attachment of all four PDFs with template message, delivery timestamp recording, retention clock start.
- Retention implementation: `lib/artifacts/fulfillment/retention.ts` with `runLocalArtifactRetention()`, `artifact_retention_candidates` RPC, and documented manual procedure until order volume justifies Vercel cron automation.
- Updated ETSY-03 / ARTIFACTS tracker status: controlled path in progress as of 2026-09-24, founder offer approved (26-page report, $34, separate Anchor Print), a11y/legal still pending Kai/Jack, publish hold. SOP and flag inventory complete; next gate is independent accessibility review + legal review closing.
- This session was docs-only: no app code changes, no migrations, no flag enabling, no product seeding, no Stripe/Etsy API calls, no Clerk user creation.
