...
| ACCESS-03 | P0 | Paid access | Update Blueprint/calendar navigation, locked states, upgrade, and cancellation behavior | in_progress | ACCESS-02 | 2–3d | B-Access | Cancellation already existed (Stripe billing portal, unchanged). Blueprint page slice shipped 2026-09-17: `currentBlueprintRowExists()` distinguishes never-generated from access-lapsed (previously both showed "Complete Setup", which was wrong for a real owned artifact); `BlueprintView` renders a windowed-access banner with an upgrade link when `capability.access === 'windowed'`. Verified against local Docker Supabase with the flag on (locked-but-exists case, 2 assertions) and off (default behavior provably unchanged, 2 assertions); `tsc`/build pass. Remaining: `/year` calendar navigation has zero capability integration — no locked states there at all | 2026-09-17 |
