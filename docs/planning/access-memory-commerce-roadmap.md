...
| SAFE-02 | P0 | Safety | Make week-reading name, scope, and persistence truthful | done | DEC-02 | 1–2d | Root-Product | Approved name/scope/persistence, no expiry read/write, one-generation guard, attribution continuity, focused regressions, full TypeScript, 99-page build, and public browser checks pass. Authenticated artifact check 2026-09-17: real Clerk sign-in (Playwright, `e2e/authenticated-evidence.spec.ts`), `/preview` confirmed as a signed-in real customer — shows "Personalized Birth-Chart Week Reading" / "Saved to your account to revisit", no trial/countdown language | 2026-09-17 |
...
| PLANNER-YEAR-01 | P0 | Paid access | Implement windowed planner year access with Dec 1 roll date | in_progress | ACCESS-01 | 3–4d | Cloud Agent | Replace CURRENT_PLANNER_YEAR constant with Dec 1 roll function; implement Option A (derive covered years from entitlement window at resolve time); add ONE_TIME_ANNUAL_ROLLS_FORWARD config switch; update Blueprint generation for multi-year access; unit tests with injected clock; update design note and end-user wiki; no DB migrations, no Stripe changes | 2026-09-24 |
...
| ETSY-03 | P1 | Etsy artifacts | Adapt admin workflow for order intake, generation, QA, and Etsy delivery | verification | ETSY-02 | 2–4d | Root-Artifact/Admin | Production now has the founder-only natal-report fulfillment console, service-role persistence, manual real-order intake, private PDF storage, QA/state transitions, and bundled serverless PDF export. `0045` and `0046` remain the exact applied persistence/package boundaries at production timestamps `20260821211738` and `20260826124803`. All three production gates are enabled, but the product seed remains inactive, all eight operational tables were empty at activation, no Etsy client exists, and no buyer/order/test fixture was created. **Operator SOP created 2026-09-24:** `docs/ops/etsy-anchor-manual-delivery-retention.md` documents production flag status (all three enabled), validator-compliant dry-run workflow (`DRYRUN-*` IDs, `dryrun.anchor@example.com`, `fictional: false` — validator rejects `.test` emails and `fictional: true` in manual intake), manual Etsy Messages delivery, and retention schedule (30/180 days/12 months/7 years). **Launch gates updated 2026-09-24:** Jack waived external accessibility and legal review for Anchor launch. Final hold: complete controlled dry-run + SOP verification → activate product + publish founder-approved $34 listing. Public a11y wording: "Designed with accessibility features; independent PDF/UA conformance is not claimed." | 2026-09-24 |
...
| `buildAnnualEntitlementRecord()` spread its `startAt` parameter into the row sent to `product_entitlements`, which has no such column. Every real one-time/annual Stripe purchase charged the customer successfully via `fulfillOneTimeCheckout`, then failed with `PGRST204` before the entitlement was created; the webhook route's catch returned 500, so Stripe retried for its standard window and then gave up — the customer paid and received nothing, silently. Confirmed via production data: 0 of 5 real entitlements have `source='stripe'` (3 `comp`, 2 legacy `etsy`); the monthly/subscription path is unaffected, it builds its payload separately without this bug | Charged customers get no product; support/refund burden; trust damage | Fixed 2026-09-17 in `lib/commerce/entitlements.ts` (destructure `startAt` out before spreading). Verified end-to-end against a real Stripe test-mode checkout completed via Playwright: payment → entitlement grant → funnel event, replay-idempotent. `tsc` and build pass | Engineering | fixed 2026-09-17; founder confirmed 2026-09-17 there has been exactly one real purchase, via Etsy (unaffected by this bug) — no customer harm occurred |
| Local Docker Supabase's `[auth.third_party.clerk]` was disabled by default, so real Clerk-issued session tokens (from a genuine sign-in) fail RLS locally even though sign-in itself succeeds — `own_journal`'s `app_current_clerk_user_id()` silently can't resolve, and a real authenticated write returns 500 with the underlying error swallowed by a bare `catch` in `app/api/journal/route.ts` that never calls `console.error` | Any authenticated-browser staging evidence (SAFE-02/CONSENT-02/ACCESS-04 and future work) silently fails on real writes, easy to misdiagnose as an app bug rather than a local config gap; the swallowed error also means production would give no diagnostic signal for a real failure of this kind | Fixed 2026-09-17: enabled `[auth.third_party.clerk]` in `supabase/config.toml` with the real dev-instance domain (decoded from `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`), now checked in. Verified: real journal write succeeds (`POST /api/journal` 201) after the fix, failed with a swallowed 500 before it. Also added `console.error` to the bare `catch` in `app/api/journal/route.ts` so a real production failure of this kind is no longer silent | Engineering | fixed |
...
| No dedicated Kairos staging project or preview branch is positively identified | Destructive cleanup and two-user RLS tests cannot be run safely | Resolved 2026-09-17: Supabase cloud branching needs a paid plan upgrade not yet made, so local Docker Supabase (`supabase init` + `supabase start`) is now the staging target — zero cost, schema/RLS-identical (`check:schema` confirms), no production data. Used already for `ACCESS-02`'s persona/RLS evidence and available for future destructive-migration work | Founder + Root | resolved 2026-09-17 |
| `0040`/`0041` were recorded here as "unapplied; blocked on staging" | Roadmap could send the next session to redo already-applied SQL, or worse, re-run destructive `0041` against live data | Correction 2026-09-17: both are already applied to production (ledger `20260915153909`/`20260915153920`, 2026-09-15 — same drift pattern already found for `0038`/`0039`/`0043`). Verified directly: `refresh_user_pattern_insight` exists in prod; `user_pattern_insights`, unedited `month_briefs`, and AI-summarized `quarterly_reviews` are all at 0 rows, consistent with `0041` having already run. `CONSENT-03`'s real remaining gap is unchanged by this — staging privacy/RLS evidence for consent enforcement/revocation, same shape as `ACCESS-02`'s now-closed gap, not the SQL itself | Root | resolved 2026-09-17 |
...
### 2026-09-17 — METRICS-02 closed; found and fixed a live checkout-fulfillment bug

- Cleared `METRICS-02`'s last gap (staged Stripe/DB reconciliation) with a real test-mode Stripe checkout: created two real `sk_test_` sessions via `createCheckoutSession`/`recordCheckoutStarted` directly (bypassing Clerk, same pattern as other staging work), completed one through Stripe's actual hosted checkout page via Playwright with the published `4242...` test card, then verified `fulfillCheckoutSession` → `recordVerifiedCheckoutCompleted` and `recordCheckoutCanceled` against local Docker Supabase, including replay idempotency on both paths. 11 assertions, 0 failures.
- That real completion surfaced a genuine bug, not a test artifact: `buildAnnualEntitlementRecord()` in `lib/commerce/entitlements.ts` spread its `startAt` window-calculation parameter straight into the `product_entitlements` upsert payload. `product_entitlements` has no `startAt` column, so PostgREST rejected every insert (`PGRST204`) — after the customer had already been charged. The webhook route's catch turned this into a 500, Stripe retried for its standard window, then gave up; the entitlement was never created. Confirmed against production data: 0 of 5 real entitlements have `source='stripe'` — every real customer so far got in via `comp` or legacy `etsy`, never through an actual Stripe purchase. The monthly/subscription path (`fulfillSubscriptionCheckout`) builds its payload separately and does not have this bug.
- Fixed by destructuring `startAt` out before the spread. Verified the fix against the real completed Stripe session end-to-end (payment → entitlement → funnel event, idempotent on replay), `tsc` clean, production build passes.
- **Founder follow-up recommended, not performed here:** check Stripe's dashboard for any real customer who completed an annual checkout and never received access, since they paid and got nothing. See the risk register.
- Cleaned up all seeded test rows and temporary scripts before finishing; no production data touched beyond the read-only entitlement-distribution query used to confirm the bug's real-world impact.
- Founder confirmed 2026-09-17: exactly one real purchase to date, via Etsy — unaffected by the bug above. No customer harm occurred; risk register updated accordingly.

### 2026-09-17 — SAFE-02 / CONSENT-02 / ACCESS-04 closed: real authenticated-browser evidence

- Installed `@clerk/testing` and `@playwright/test`. Found Clerk's existing dev-instance test user (`mara@example.com`, `publicMetadata.isDemo: true`) rather than creating a new one; the founder's own real account was identified and left untouched.
- `scripts/seed-e2e-persona.mts` (promoted from a throwaway script) seeds Mara into local Docker Supabase as an active annual planner+oracle persona with a ready Blueprint and a ready week preview.
- `e2e/authenticated-evidence.spec.ts` + `playwright.config.ts` + `e2e/global-setup.ts`: real Clerk sign-in via `clerk.signIn({ emailAddress })` (ticket-based, no password needed), then three real checks against a dev server running on local Docker Supabase — never production:
  - **SAFE-02**: `/preview` shows the approved truthful name/persistence copy, no trial/countdown language.
  - **CONSENT-02**: the legacy consent toggle is visible; a real entry saves (`POST /api/journal` 201) and is confirmed persisted.
  - **ACCESS-04**: `/settings` shows a working billing control; `/pricing` doesn't tell an already-top-tier customer to upgrade.
- The CONSENT-02 check surfaced a real, non-obvious environment gap, not an app bug: local Supabase's `[auth.third_party.clerk]` was disabled by default, so a genuine Clerk sign-in succeeded but the resulting session token failed RLS on write, and the failure was completely silent — `app/api/journal/route.ts`'s catch block never logged the underlying error. Fixed both: enabled the real Clerk domain in `supabase/config.toml` (checked in, benefits all future local/E2E work), and added the missing `console.error` so a similar failure is never silent again. Confirmed the exact same real Playwright sign-in + save now works end-to-end, stable across repeated clean runs.
...
### 2026-09-24 — Etsy Anchor controlled path: flags documented, operator SOP created

- Inventoried all Etsy/artifact feature flags, admin access layers, and intake/delivery/publication controls. Created `docs/ops/etsy-anchor-manual-delivery-retention.md` covering:
  - **Production status verified 2026-09-24:** All three artifact flags (`KIAROS_ETSY_ARTIFACT_ADMIN`, `KIAROS_ETSY_ARTIFACT_PERSISTENCE`, `KIAROS_ETSY_ARTIFACT_REAL_INTAKE`) enabled in Vercel production. Product `KAI-ETSY-ANCHOR-V1` remains inactive (`active=false`). Etsy listing remains unpublished (draft on Seller Dashboard).
  - Named operator enforcement via Clerk `publicMetadata.isAdmin` (no separate env allowlist) and `artifact_admin_roles` table (schema exists, not actively enforced yet).
  - Admin UI location: `app/(admin)/admin/artifacts` → `/admin/artifacts` (durable on main). API route: `/api/admin/artifacts/[orderId]/actions`.
  - Production dry-run click path for manual intake using validator-compliant fake data: `DRYRUN-*` IDs (not `fixture-*`), `dryrun.anchor@example.com` (not `.test`), `fictional: false` (validator rejects `fictional: true` in `validateManualArtifactIntake`). Local fixtures remain dev-only.
  - Manual Etsy Messages delivery protocol: what to attach (4 PDFs), template message, post-delivery actions.
  - Retention schedule aligned with Privacy/Terms: 30 days raw personalization, 180 days normalized/files, 12 months support, 7 years minimal ledger. Manual `runLocalArtifactRetention()` until automation justified.
  - **Launch gates updated 2026-09-24:** Jack waived waiting on external accessibility and legal review for Anchor launch. New hold: complete controlled dry-run + this SOP verification, then activate product + publish founder-approved $34 listing. Public a11y wording: "Designed with accessibility features; independent PDF/UA conformance is not claimed."
- Three artifact flags inventoried and production status verified: all `"true"` in Vercel production. Product activation + listing publication held until controlled dry-run + SOP verification complete.
- Four legacy Etsy SKU/listing mapping vars (`ETSY_SKU_PLANNER`, `ETSY_SKU_PLANNER_ORACLE`, `ETSY_LISTING_ID_*`) and `N8N_INGEST_SECRET` documented as separate activation path, not artifact fulfillment.
- Production dry-run workflow: manual intake (`fictional: false`) with `DRYRUN-SHOP-001` / `DRYRUN-RECEIPT-001` / `DRYRUN-TX-001` + `dryrun.anchor@example.com` → validate → generate → QA (6 items) → export (4 PDFs) → download for review. No product activation, no listing publication yet.
- Validator accuracy confirmed: `validateManualArtifactIntake()` in `lib/artifacts/fulfillment/workflow.ts` lines 152-168 rejects `fictional: true`, `fixture-*` IDs, `.test` emails, and `art_fixture_*` artifact IDs. SOP updated to match actual validator logic.
- Retention implementation: `lib/artifacts/fulfillment/retention.ts` with `runLocalArtifactRetention()`, `artifact_retention_candidates` RPC, manual procedure documented.
- Updated ETSY-03 / ARTIFACTS tracker status: controlled path in progress, founder offer approved (26-page report, $34), external a11y/legal waived by Jack 2026-09-24, dry-run + SOP verification now final gate before product activation + listing publication.
- This session was docs-only: no app code changes, no migrations, no flag changes (already enabled), no product activation, no listing publication.
